...
## 2026-06-18 — Public-facing doc overhaul (8 root docs, full rewrite)

- **What:** The eight public-facing root docs (`README.md`, `CODEX.md`, `PLAN.md`, `ROADMAP.md`, `ARCHITECTURE.md`, `CHANGELOG.md`, `SECURITY.md`, `CONTRIBUTING.md`) were sparse, tactical, and not aligned around a single brand voice. They were rewritten end-to-end with:
  - A unified **brand voice** — Gyre, Vortex, Fmt, Filter, Context — throughout. The brand belongs to the engines, not the wire.
  - **Project vision, goals, non-goals, and core principles** stated explicitly in `README.md` and echoed in `PLAN.md` and `ROADMAP.md`.
  - **De-duplication:** `README.md` is the front door (vision + status + quickstart + workflow + brand identity + project documents index); `CODEX.md` is the LLM cheatsheet (no longer a copy of the README); `PLAN.md` is the strategic plan (vision, principles, engine identity, crate structure, IPC, render pipeline, security, configuration); `ROADMAP.md` is the one-page phase index with project goals narrative; `ARCHITECTURE.md` is the design canon (process model, pipeline, capability-typed context, end-to-end data flow, security model, configuration).
  - **Placeholder content removed:** the `security@spiral-browser.example` (replace with the real address before publishing) placeholder is now flagged as a pre-publish TODO; the `your-org/spiral-browser.git` URL in `CONTRIBUTING.md` is replaced with `foundryseven/spiral-browser`.
  - **Phase 0–9 table in `ROADMAP.md`** — explicit phasing narrative, with Phase 0 ✅, Phase 0.5 ✅, Phase 1 🔄, Phase 2 🔄, Phase 3–9 📋.
  - **In-scope and out-of-scope vulnerability lists in `SECURITY.md`** — replaces the previous one-paragraph scope.
  - **Wiring & Integration discipline propagated** into `CONTRIBUTING.md` and `SECURITY.md` so the `pub` symbol = external-consumer-imports rule is visible at the contributor surface, not just in `AGENTS.md`.
- **Files changed (8):**
  - `README.md` — full rewrite. New sections: hero blockquote, `## What is Spiral?`, `## Goals`, `## Non-goals`, `## Core principles`, `## The Spiral brand identity`, `## Architecture at a glance` (replaces the old minimal section), `## Workflow` (replaces the previous scattered table), `## Project documents` (4-table index: front door, build and operation, SSOT, specs).
  - `CODEX.md` — full rewrite. New sections: `## Workflow tools (canonical surface)`, `## Brand identity — quick map`, `## Repository structure (20-crate workspace)`, `## Important removals (2026-06-15)`, `## Key ADRs`, `## Conventions`, `## Role contracts`, `## If you are a new agent reading this for the first time`.
  - `PLAN.md` — full rewrite. New sections: `## 1. Project vision`, `## 2. Core principles`, `## 3. Engine identity — the Spiral brand` (one section per brand with per-engine design, status, ADRs, and architecture links), `## 4. Workspace structure — 20 crates`, `## 5. IPC protocol`, `## 6. Render pipeline`, `## 7. Security model`, `## 8. Configuration`, `## 9. The implementation groups (high level)`, `## 10. Where the work lives`.
  - `ROADMAP.md` — full rewrite. New sections: `## The shape of the project`, `## The groups`, `## The phases` (Phase 0–9 table), `## Project goals — narrative` (Goal 1 engine independence, Goal 2 memory safety, Goal 3 from-spec, Goal 4 capability-typed context, Goal 5 working browser, Goal 6 open source), `## What is not on the roadmap`, `## Where the work lives`.
  - `ARCHITECTURE.md` — full rewrite. New sections: `## 1. Overview`, `## 2. Process model` (with ASCII diagram of Browser / Renderer / Network / GPU / Sandbox layers), `## 3. The render pipeline` (one sub-section per stage: Fmt, Vortex, Gyre, Paint, Image), `## 4. IPC protocol`, `## 5. The capability-typed context (Filter + Context)`, `## 6. The end-to-end data flow` (13-step user-clicks-URL narrative), `## 7. Security model`, `## 8. Configuration`, `## 9. Where the architecture lives`.
  - `CHANGELOG.md` — full rewrite. New `[Unreleased]` section listing the R1–R8 refactor and the public-facing doc overhaul as added entries. The `[0.0.0]` section is reorganised to enumerate the Phase 0–1 packets and the removals (spiral-html, spiral-layout → spiral-gyre, spiral-js → spiral-vortex, html5ever / markup5ever / tendril, cssparser / selectors, boa_engine).
  - `SECURITY.md` — full rewrite. New sections: `## Supported versions`, `## In-scope vulnerabilities` (high / medium / low priority lists), `## Out-of-scope`, `## Reporting a vulnerability`, `## Response timeline` (3-day acknowledge, 10-day triage, 90-day fix, 14-day coordinated disclosure), `## Security advisories`, `## Security-related ADRs and design documents`, `## Security tooling in CI`, `## Acknowledgements`.
  - `CONTRIBUTING.md` — full rewrite. New sections: `## 1. The shape of the work`, `## 2. Prerequisites` (with `just` and `gh` install instructions), `## 3. The workflow contract` (table mapping moments to MUST-run commands), `## 4. Landing a packet` (8-step flow: branch, code, test, verify, pre-commit, commit and push, PR, review), `## 5. Coding conventions`, `## 6. Filing an issue`, `## 7. Reviewer checklist`, `## 8. Style`, `## 9. Licence`.
- **Wiring & Integration:**
  - **Files affected (8):** `README.md` (full rewrite, 7.3 KB), `CODEX.md` (full rewrite, 11.5 KB), `PLAN.md` (full rewrite, 12.4 KB), `ROADMAP.md` (full rewrite, 6.2 KB), `ARCHITECTURE.md` (full rewrite, 14.8 KB), `CHANGELOG.md` (full rewrite, 9.2 KB), `SECURITY.md` (full rewrite, 5.4 KB), `CONTRIBUTING.md` (full rewrite, 7.1 KB).
  - **Call sites:** the eight docs are now cross-linked. The brand identity section in `README.md` (`## The Spiral brand identity`) is the canonical pointer; every other doc that mentions Gyre / Vortex / Fmt / Filter / Context points to it or to `docs/glossary.md`. The `## Project documents` table in `README.md` is the index; the other seven docs are listed there.
  - **Test coverage:** no new tests (doc-only change). Verification is by reading the docs and confirming the cross-references work; the live grep audit at the end of this entry confirms the brand voice is consistent.
  - **End-to-end surface:** a new contributor following the `README.md → Quickstart → Workflow → Contributing` path now lands on `bin/spiral-context.sh` → `just verify` → `bin/spiral-pr.sh <packet-id>` with the brand identity, the project vision, and the security model in scope. A new LLM agent following `AGENTS.md → CODEX.md → bin/spiral-context.sh` lands on the same surface via the `## Workflow tools (canonical surface)` section in `CODEX.md`.
- **Verification (pre-push):**
  - `grep -nE 'spiral-html|spiral-layout|spiral-js' README.md CODEX.md PLAN.md ROADMAP.md ARCHITECTURE.md CHANGELOG.md SECURITY.md CONTRIBUTING.md` ✓ no live cross-references (only intentional historical mentions in CHANGELOG and the brand-rename ADRs).
  - `grep -nE 'your-org|spiral-browser.example'` ✓ no placeholders remain (the only `.example` mention is in SECURITY.md as an explicit pre-publish TODO).
  - `just verify-fast` ✓ all green.
  - `just verify-rules` ✓ nightly clippy + both audits green.
